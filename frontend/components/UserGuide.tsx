...
print(resp.choices[0].message.content)`;
  ...
  return (
    <section className="ug">
      <h2>新手操作手册（上手指南）</h2>
      <p className="lead">
        左侧菜单从上到下是：操作手册 → 科研见证 → 仪表盘 → API 调用记录 → 合规管理 → 行为监控 →
...
└──────────────────┘`}
      ...
      <ol className="steps">
        ...
        <li>
          <h3>第五步：科研优先权存证（推荐在调用 AI 之前做）</h3>
          <p>
            把未发表草稿或提示词发给任何 AI 之前，可以先留下指纹。系统<strong>只保存 64 位哈希</strong>
            ，原文不出网。这是技术存证，不是法院判决。
          </p>
          <ol className="sub">
            <li>
              点左侧菜单 <strong>「科研见证」</strong>
            </li>
            <li>
            ...
            <li>
              等页面显示 <strong>64 位哈希</strong> 后，再点 <strong>「写入证据链」</strong>
            </li>
            <li>
              在列表里点开该条（或直接点行内按钮），再点 <strong>「导出优先权证书」</strong>
              ，下载 ZIP（文件名类似 <code>ata_priority_…_cert.zip</code>）。请与原件放在同一加密盘。
            </li>
          </ol>
          <p>离线打开证书（断网也可）：</p>
          <ol className="sub">
            ...
            <li>
              点 <strong>「运行本地验证」</strong>
            </li>
            <li>
              如需核对原件：点 <strong>「用原件选择文件」</strong> 选中当时那份文件。改一个字会显示不匹配。
            </li>
          </ol>
          <p className="tip">
